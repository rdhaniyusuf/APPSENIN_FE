...
const SidebarComp: FC<SidebarProps> = () => {
  ...
  return (
    <>
      ...
      <div
        className={`${
          isOpen ? "w-64" : "w-16 sm:items-center"
...
function NavItem({ href, icon, text, isOpen, isActive }: NavItemProps) {
  return isOpen ? (
    <Link
      href={href}
      className={`flex items-center space-x-4 p-3 hover:bg-gray-300 rounded-md ${
        isActive
          ? "bg-slate-400 drop-shadow-sm text-cyan-900 rounded-md hover:cursor-default"
          : ""
      }`}
    >
      {icon}
